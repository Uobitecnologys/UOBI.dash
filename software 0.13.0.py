...
def apply_theme():
    ...
    if SETTINGS["theme"] == "Hacker Green":
        blue = green = cyan = yellow = magenta = white = "\033[92m"
        red = "\033[90m"
    elif SETTINGS["theme"] == "Matrix Red":
...
clear_screen()
password = input(f"{yellow}Enter Password : ")
...
while True:
    ...
    if action == "1":
        ...
        print("=" * 20)
        target = input(f"{red}Enter Target IP/Domain : ")
        ports = [10, 80, 443, 433, 8080]
        ...
        try:
            for port in ports:
                ...
                if result == 0:
                    print(f"{green}Port :{port:4} Open{RESET}")
                else:
                    print(f"{red}Port :{port:4} Closed{RESET}")
                s.close()
        except Exception as e:
        ...
        while True:
            ...
            print(f"2. Change Color Theme [{SETTINGS['theme']}]")
            ...
            if set_choice == "1":
                old_p = input(f"\n{yellow}Enter Current Password: {RESET}")
                ...
                if th == "2": SETTINGS["theme"] = "Hacker Green"
                elif th == "3": SETTINGS["theme"] = "Matrix Red"
